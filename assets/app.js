...
(function () {
  ...
  if (тарифы.length) {
    ...
    /* ---------- Платёж отклонён ----------
       Platega возвращает человека сюда с ?failed=номер. До сих пор он
       видел ровно ту же форму, что и до оплаты, — ни слова о том, что
       случилось и где деньги. Показываем блок над формой; повторить
       попытку можно тут же, отдельного шага не требуется.

       Метку из адреса стираем сразу: иначе обновление страницы снова
       показало бы отказ, которого уже нет, а человек решил бы, что
       вторая попытка тоже сорвалась. */
    var отказ = (location.search.match(/[?&]failed=([^&]+)/) || [])[1];
    if (отказ) {
      var блокОтказа = document.querySelector('[data-fail]');
      if (блокОтказа) {
        var номерОтказа = блокОтказа.querySelector('[data-fail-num]');
        if (номерОтказа) номерОтказа.textContent = 'Заказ № ' + decodeURIComponent(отказ);
        блокОтказа.hidden = false;
      }
    ...
    if (кнопка) кнопка.addEventListener('click', function () {
      ...
      /* Кнопка не просто гаснет и меняет надпись: на медленной сети это
         неотличимо от зависшей страницы — человек жмёт ещё раз или уходит.
         Признак работы рисует сама кнопка по метке aria-busy — полосой
         по нижнему краю. Отдельного значка нет намеренно: круглых форм
         в системе Lanelis не бывает, а в расширении ожидание тоже
         показано изменением самого элемента (favpulse у иконок), а не
         вертушкой рядом. Метка при этом не украшение: читалка экрана
         по ней объявляет, что кнопка занята. */
      var было = кнопка.innerHTML;
      кнопка.disabled = true;
      кнопка.setAttribute('aria-busy', 'true');
      ...
      fetch('/api/checkout', {
        ...
        .then(function (о) {
          ...
          кнопка.removeAttribute('aria-busy');
          сказать(
  ...
  if (экран) {
    ...
    /* Плашки ожидания живут ровно до прихода значения. Если ответа так и
       не будет — после пятнадцати попыток или при обрыве связи — они
       обязаны исчезнуть: вечно мерцающая плашка обещает то, чего уже
       не случится, и это хуже прочерка. Ставим прочерк — он честно
       говорит «значения нет». */
    var сдаться = function () {
      ['plan', 'sum', 'till'].forEach(function (имя) {
        var el = поле(имя);
        if (el && el.querySelector('.skl')) el.textContent = '—';
      });
    };
    ...
    var спросить = function () {
      ...
      fetch('/api/order?n=' + encodeURIComponent(номер))
        ...
        .catch(function () {
          if (++попыток < 15) { setTimeout(спросить, 2000); return; }
          вПоле('h', 'Оплата принята');
          вПоле('sub', 'Не удалось получить подробности заказа. Доступ это не отменяет — '
                     + 'откройте новую вкладку. Если его нет, напишите нам номер заказа.');
          сдаться();
        });
    };
    спросить();
